...
import { SolucoesTab } from "@/components/crm/SolucoesTab";
...
function CrmPage() {
  ...
  const filtered = useMemo(() => {
    return leads.filter((l) => {
      ...
      if (query) {
        ...
        return [l.nome, l.whatsapp, l.tipo_negocio, l.cnpj].some((v) =>
  ...
  const actions = (
    <button
      ...
      className="inline-flex items-center gap-2 rounded-lg border border-border px-3 py-1.5 text-xs font-bold hover:border-primary hover:text-primary"
    >
  ...
  return (
    <CrmShell
      ...
      subtitle="Trabalhe cada cliente nas 5 etapas — do Diagnóstico aos Resultados."
      actions={actions}
    >
      <div className="mx-auto max-w-7xl px-6 py-8">
        <h1 className="text-3xl md:text-4xl font-bold">Funil de clientes</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Trabalhe cada cliente nas 5 etapas — do Diagnóstico aos Resultados.
        </p>
        ...
        {/* Table */}
        <div
          className="mt-6 rounded-2xl border border-border bg-card/60 backdrop-blur overflow-hidden"
                ...
                {loading ? (
                  ...
                  filtered.map((l) => (
                    <tr
                      ...
                      className="border-t border-border/40 cursor-pointer hover:bg-secondary/30 transition"
                    >
                      <td className="px-4 py-3">
                        <div className="font-semibold">{l.nome}</div>
                        {l.cnpj && (
                          <div className="text-xs text-muted-foreground mt-0.5">CNPJ: {l.cnpj}</div>
                        )}
                      </td>
...
function StageChip({
  ...
  return (
    <button
      onClick={onClick}
      className={`inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-semibold uppercase tracking-wider transition border ${active ? "bg-primary text-primary-foreground border-primary" : "bg-background/40 text-muted-foreground border-border hover:border-primary/50 hover:text-foreground"}`}
    >
      {icon}
      {label}
      <span
        className={`rounded-full px-2 py-0.5 text-[0.65rem] ${active ? "bg-primary-foreground/20" : "bg-secondary"}`}
      >
        {count}
      </span>
    </button>
...
function LeadDrawer({
  ...
  const [tab, setTab] = useState<"dados" | "solucoes" | Stage>(lead.stage);
  ...
  async function runAI<T>(key: string, fn: () => Promise<T>, apply: (r: T) => void) {
    setAiLoading(key);
    setAiError(null);
    try {
      apply(await fn());
  ...
  async function exportPDF() {
    ...
    const impactBadge = (impacto: string) => {
      ...
      if (i === "alto")
        return { fill: [253, 230, 230] as [number, number, number], text: C.red, label: "Alto" };
      if (i === "medio")
        return { fill: [255, 243, 219] as [number, number, number], text: C.amber, label: "Médio" };
      if (i === "baixo")
        return { fill: [232, 246, 237] as [number, number, number], text: C.green, label: "Baixo" };
    ...
    const priorityBadge = (prioridade: string) => {
      ...
      if (p === "alta")
        return { fill: [253, 230, 230] as [number, number, number], text: C.red, label: "Alta" };
      if (p === "media")
        return { fill: [255, 243, 219] as [number, number, number], text: C.amber, label: "Média" };
      if (p === "baixa")
        return { fill: [232, 246, 237] as [number, number, number], text: C.green, label: "Baixa" };
      return {
    ...
    const sectionTitle = (
      num: number,
      title: string,
      tone: [number, number, number] = C.primary,
    ) => {
      ensure(58);
      y += 8;
      fill(tone);
    ...
    const card = (
      ...
      w: number,
      h: number,
      fillColor: [number, number, number] = C.paper,
    ) => {
      fill(fillColor);
      stroke(C.border);
    ...
    const tocItems = [
      ...
      { n: "02", title: "Análise", on: !!draft.analise_ai },
      {
        n: "03",
        title: "Oportunidades",
        on: (draft.oportunidades ?? []).some((o) => o.selecionada),
      },
    ...
    const renderOpportunity = (o: Oportunidade, index: number) => {
      ...
      drawWrappedLines(titleLines, M + 52, y + 28, {
        size: 12.2,
        color: C.ink,
        lineHeight: 15,
        bold: true,
    ...
    const renderAction = (a: Acao, index: number) => {
      ...
      const height =
        34 + titleLines.length * 15 + metaLines.length * 12 + descLines.length * 13 + 24;
      ensure(height + 12);
      card(M, y, CONTENT_W, height);
      fill(C.primary);
      ...
      drawWrappedLines(titleLines, M + 18, y + 39, {
        size: 12.4,
        color: C.ink,
        lineHeight: 15,
        bold: true,
      });
      drawWrappedLines(metaLines, M + 18, y + 39 + titleLines.length * 15 + 8, {
        size: 9.2,
        color: C.sub,
        lineHeight: 12,
      });
      drawWrappedLines(
        descLines,
        M + 18,
        y + 39 + titleLines.length * 15 + 8 + metaLines.length * 12 + 8,
        {
          size: 10.1,
          color: C.ink,
          lineHeight: 13,
    ...
    const renderMetric = (m: Metrica, index: number) => {
      ...
      drawWrappedLines(nameLines, M + 48, y + 26, {
        size: 11.4,
        color: C.ink,
        lineHeight: 12,
        bold: true,
      ...
      setFont(8.8, false);
      doc.text(
        `${safeText(m.unidade)} · Atual: ${safeText(m.valor_atual)}`,
        M + 48,
        y + height - 14,
      );
      y += height + 12;
    ...
    drawWrappedLines(titleLines, M + 160, y + 56, {
      size: 22,
      color: C.primaryDark,
      lineHeight: 26,
      bold: true,
    ...
    if (draft.desafios_reais || draft.objetivos_organizacionais) {
      ...
      if (draft.desafios_reais) {
        renderFlowText("Desafios reais", draft.desafios_reais, {
          bodySize: 10.15,
          lineGap: 4.6,
          tone: C.accent,
        });
      ...
      if (draft.objetivos_organizacionais) {
        renderFlowText("Objetivos organizacionais", draft.objetivos_organizacionais, {
          bodySize: 10.15,
          lineGap: 4.6,
          tone: C.primary,
        });
    ...
    if (draft.diagnostico_ai) {
      ...
      renderFlowText("Leitura estratégica", draft.diagnostico_ai, {
        bodySize: 10.15,
        lineGap: 4.6,
        tone: C.primary,
      });
    ...
    if (draft.analise_ai) {
      ...
      renderFlowText("Síntese analítica", draft.analise_ai, {
        bodySize: 10.15,
        lineGap: 4.6,
        tone: C.accent,
      });
      ...
      >
          ...
          <div className="flex items-start justify-between gap-3 pl-2">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                title={isMaximized ? "Restaurar largura" : "Expandir painel"}
        ...
        <div className="p-6 space-y-5 flex-1 overflow-y-auto pl-8">
          ...
          {tab === "dados" && (
            <>
              <Field label="Etapa do funil">
                <div className="grid grid-cols-5 gap-1.5">
                  {STAGES.map((s) => (
                    <button
                      key={s.value}
                      onClick={() => setDraft({ ...draft, stage: s.value })}
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Nome">
                  <Input value={draft.nome} onChange={(v) => setDraft({ ...draft, nome: v })} />
                </Field>
                ...
                <Field label="Tipo de negócio">
                  <Input
                    value={draft.tipo_negocio ?? ""}
                    onChange={(v) => setDraft({ ...draft, tipo_negocio: v })}
                  />
                </Field>
                <Field label="CNPJ">
                  <Input
                    value={draft.cnpj ?? ""}
                    onChange={(v) => setDraft({ ...draft, cnpj: v })}
                  />
                </Field>
              </div>
              <Field label="Soluções prestadas">
                <Textarea
                  rows={4}
                  value={draft.solucoes_prestadas ?? ""}
                  onChange={(v) => setDraft({ ...draft, solucoes_prestadas: v })}
                />
              </Field>
              <Field label="Desafios reais">
                <Textarea
                  value={draft.desafios_reais ?? ""}
                  onChange={(v) => setDraft({ ...draft, desafios_reais: v })}
                />
              </Field>
              <Field label="Objetivos organizacionais">
                <Textarea
                  value={draft.objetivos_organizacionais ?? ""}
                  onChange={(v) => setDraft({ ...draft, objetivos_organizacionais: v })}
                />
              </Field>
            </>
          )}
            ...
            <>
              <Field label="Anotações do consultor (insumo da IA)">
                <Textarea
                  ...
                  onChange={(v) => setDraft({ ...draft, anotacoes: v })}
                />
              </Field>
              <AIButton
                loading={aiLoading === "diag"}
                onClick={() =>
                  runAI(
                    "diag",
                    () => runEnrich({ data: { leadId: lead.id } }),
                    (r) =>
                      setDraft({
                        ...draft,
                ...
                label={
                  draft.diagnostico_ai
                    ? "Reprocessar diagnóstico com IA"
                    : "Enriquecer diagnóstico com IA"
                }
              ...
              <Field label="Diagnóstico enriquecido">
                <Textarea
                  rows={12}
                  value={draft.diagnostico_ai ?? ""}
                  onChange={(v) => setDraft({ ...draft, diagnostico_ai: v })}
                />
              </Field>
            </>
          )}
          ...
          {tab === "analise" && (
            <>
              <AIButton
                loading={aiLoading === "ana"}
                disabled={!draft.diagnostico_ai}
                onClick={() =>
                  runAI(
                    "ana",
                    () => runAnalise({ data: { leadId: lead.id } }),
                    (r) =>
                      setDraft({
                        ...draft,
                ...
                hint={
                  draft.diagnostico_ai
                    ? "Transborda o diagnóstico em análise detalhada"
                    : "Gere o diagnóstico antes"
                }
              />
              <Field label="Análise detalhada">
                <Textarea
                  rows={10}
                  value={draft.analise_ai ?? ""}
                  onChange={(v) => setDraft({ ...draft, analise_ai: v })}
                />
              </Field>
              <div>
                <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-2">
                  <Target className="h-3.5 w-3.5" /> Oportunidades encontradas
                </label>
                <div className="mt-2 space-y-2">
                  {(draft.oportunidades ?? []).length === 0 && (
                    <p className="text-xs text-muted-foreground">
                      Nenhuma oportunidade ainda. Gere a análise.
                    </p>
                  )}
                  {(draft.oportunidades ?? []).map((o, i) => (
                    <label
                      key={i}
                      className={`flex gap-3 rounded-lg border p-3 cursor-pointer transition ${o.selecionada ? "border-primary bg-primary/5" : "border-border hover:border-primary/40"}`}
                    >
                      <input
                        type="checkbox"
                        checked={!!o.selecionada}
                        onChange={(e) => {
                          const next = [...draft.oportunidades];
                          next[i] = { ...o, selecionada: e.target.checked };
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span
                            className={`inline-flex items-center rounded-full border px-1.5 py-0.5 text-[0.6rem] font-bold uppercase tracking-wider ${getImpactoStyles(o.impacto).badge}`}
                          >
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <h3 className="mt-1 font-semibold text-sm">
                        Mapeamento de oferta e contexto
                      </h3>
                    </div>
                    <button
                    ...
                    {needsDiscovery ? (
                      <>
                        ...
                        <p className="mt-1 text-xs text-muted-foreground">
                          Quando o cliente ainda não tiver esse mapeamento, usamos diagnóstico e
                          análise para seguir com a estratégia e deixamos esse bloco como contexto
                          opcional.
                        </p>
                      </>
                    ) : (
                ...
                <div className="rounded-2xl border border-border bg-card/60 p-4">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <h3 className="mt-1 font-semibold text-sm">
                        Plano gerado a partir do diagnóstico e da análise
                      </h3>
                    </div>
                  ...
                  <div className="mt-4">
                    <AIButton
                      loading={aiLoading === "plano"}
                      disabled={
                        (draft.oportunidades ?? []).filter((o) => o.selecionada).length === 0
                      }
                      onClick={() =>
                        runAI(
                          "plano",
                          () => runPlano({ data: { leadId: lead.id } }),
                          (r) =>
                            setDraft({ ...draft, plano_acoes: r.plano_acoes, stage: "estrategia" }),
                        )
                      }
                      label={
                        draft.plano_acoes?.length
                          ? "Reprocessar plano"
                          : "Gerar plano de ação personalizado"
                      }
                      hint={
                        needsDiscovery
                          ? "Começa pelo discovery quando o mapa ainda não está fechado"
                          : "Com base nas oportunidades selecionadas"
                      }
                    />
                  </div>
                      ...
                      <div key={i} className="rounded-lg border border-border p-3">
                        <div className="flex items-center justify-between gap-2">
                          <div className="font-semibold text-sm">
                            <span className="text-primary">{i + 1}.</span> {a.titulo}
                          </div>
                          <span
                            className={`inline-flex items-center rounded-full border px-1.5 py-0.5 text-[0.6rem] font-bold uppercase tracking-wider ${getPrioridadeStyles(a.prioridade).badge}`}
                          >
                            {getPrioridadeStyles(a.prioridade).label}
                          </span>
                        </div>
                        <div className="text-[0.65rem] uppercase tracking-wider text-muted-foreground mt-1">
                          Prazo: {a.prazo} · Responsável: {a.responsavel}
                        </div>
                  ...
                  {draft.plano_acoes?.length > 0 && (
                    <button
                      onClick={exportPDF}
                      className="mt-4 w-full inline-flex items-center justify-center gap-2 rounded-lg bg-primary text-primary-foreground font-bold px-4 py-3 hover:opacity-90"
                    >
            ...
            <>
              <div className="rounded-lg border border-border p-3 text-xs text-muted-foreground">
                <p className="font-semibold text-foreground mb-1 flex items-center gap-1.5">
                  <Rocket className="h-3.5 w-3.5 text-primary" /> Acompanhamento da execução
                </p>
              ...
              <div className="space-y-2">
                {(draft.plano_acoes ?? []).map((a, i) => (
                  <label
                    key={i}
                    className={`flex items-start gap-3 rounded-lg border p-3 cursor-pointer transition ${a.concluida ? "border-primary bg-primary/5" : "border-border"}`}
                  >
                    <input
                      type="checkbox"
                      checked={!!a.concluida}
                      onChange={(e) => {
                        ...
                        next[i] = { ...a, concluida: e.target.checked };
                    ...
                    <div className="flex-1">
                      <div
                        className={`font-semibold text-sm ${a.concluida ? "line-through text-muted-foreground" : ""}`}
                      >
                        {a.titulo}
                      </div>
                      <div className="text-[0.65rem] uppercase tracking-wider text-muted-foreground mt-0.5">
                        {a.prazo} · {a.responsavel}
                      </div>
                    </div>
          ...
          {tab === "resultados" && (
            <>
              <AIButton
                loading={aiLoading === "metricas"}
                disabled={(draft.plano_acoes ?? []).length === 0}
                onClick={() =>
                  runAI(
                    "metricas",
                    () => runMetricas({ data: { leadId: lead.id } }),
                    (r) =>
                      setDraft({
                        ...draft,
                ...
                label={
                  draft.resultados_metricas?.length
                    ? "Atualizar métricas sugeridas"
                    : "Sugerir métricas com IA"
                }
                  ...
                  <div key={i} className="rounded-lg border border-border p-3 space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      <div className="font-semibold text-sm flex items-center gap-1.5">
                        <TrendingUp className="h-3.5 w-3.5 text-primary" /> {m.nome}
                      </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 text-xs">
                      <div>
                        ...
                        <div className="font-semibold">
                          {m.meta}{" "}
                          <span className="text-muted-foreground font-normal">{m.unidade}</span>
                        </div>
                      </div>
                      <div className="col-span-2">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            next[i] = { ...m, valor_atual: e.target.value };
                ...
                <button
                  onClick={() =>
                    setDraft({
                      ...draft,
                      resultados_metricas: [
                        ...(draft.resultados_metricas ?? []),
                        { nome: "", unidade: "", frequencia: "Semanal", meta: "", valor_atual: "" },
                      ],
                    })
                  }
                  className="w-full inline-flex items-center justify-center gap-1.5 rounded-lg border border-dashed border-border py-2 text-xs text-muted-foreground hover:border-primary hover:text-primary"
                >
              ...
              <Field label="Observações de acompanhamento e ajustes">
                <Textarea
                  rows={5}
                  value={draft.resultados_notas ?? ""}
                  onChange={(v) => setDraft({ ...draft, resultados_notas: v })}
                />
              </Field>
            </>
          )}

          {tab === "solucoes" && <SolucoesTab leadId={lead.id} />}
          ...
          <div className="flex items-center justify-between pt-4 border-t border-border">
            ...
            <button
              onClick={save}
              disabled={saving}
              className="inline-flex items-center gap-2 rounded-lg bg-primary text-primary-foreground font-bold px-5 py-2.5 hover:opacity-90 transition disabled:opacity-60"
            >
...
function TabBtn({
  ...
  return (
    <button
      onClick={onClick}
      className={`inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold uppercase tracking-wider whitespace-nowrap border-b-2 transition ${active ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}
    >
      {icon} {label}
    </button>
  );
  ...
  return (
    <div className="rounded-xl border border-primary/30 bg-primary/5 p-3">
      <button
        ...
        className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-primary text-primary-foreground font-bold px-4 py-2.5 hover:opacity-90 transition disabled:opacity-50"
      >
